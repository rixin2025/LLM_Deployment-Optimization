...
class ONNX_TRT:
    ...
    def export_onnx(self, onnx_file_path, pretrained_model_path, image_url):
        ...
        torch.onnx.export(
            ...
            use_external_data_format=False  # 关键参数：避免使用外部数据格式
        )
    ...
    def generate_trt_engine(self,
        ...
        # 为RTX 2080 Ti优化配置
        config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, 4 << 30)  # 4GB (适应11GB显存)
        
        # 启用FP16（RTX 2080 Ti支持）
        config.set_flag(trt.BuilderFlag.FP16)
        
        # 禁用不兼容的功能
        config.clear_flag(trt.BuilderFlag.TF32)  # RTX 2080 Ti不支持TF32
        ...
        nMinBS = minBS
        nOptBS = optBS
        nMaxBS = maxBS  # 减小最大批量大小
        inputT = network.get_input(0)
        inputT.shape = [nBS, 3, self.image_size, self.image_size]
        profile.set_shape(
            inputT.name,
            [nMinBS, 3, self.image_size, self.image_size],  # 更小的最小批量
            [nOptBS, 3, self.image_size, self.image_size],  # 更小的优化批量
            [nMaxBS, 3, self.image_size, self.image_size],  # 更小的最大批量
        )
